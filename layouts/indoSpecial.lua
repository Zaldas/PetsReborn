-- layouts/indoSpecial.lua
-- Hybrid layout: XIV-style backgrounds with FFXI-style bars/fonts.
-- Status effects rendered as a horizontal strip below the recast rows.
-- All positions/sizes are in D3D menu pixels (at FFXI's native internal resolution).
-- Layout values are authored at ~768p FFXI internal (half of 1440p screen).
-- The rendering pipeline uses absoluteScale=1 so layout values map directly to D3D coords.
--
-- ANCHOR SYSTEM
-- Elements may declare:
--   anchor = 'elementName'            -- resolve pos relative to that element's origin
--   anchor = {'first', 'second', ...} -- try each in order; first registered wins
-- If anchor is nil/'none', pos is relative to the window origin (legacy behavior).
-- pos is always an OFFSET from the resolved anchor, not an absolute coordinate.
-- Special axis form: 'name.bottom' resolves to {x=name.x, y=name.y+name.h} (bottom edge).

return {
    -- Target bar: shown directly below the pet window when pet has a target.
    -- anchor='window.bottom' positions it flush below the pet window (resolved via posRegistry).
    -- XIV-style background; FFXI-style bar tint colors.
    targetFrame = {
        anchor = 'window.bottom',
        pos    = {0, 0},   -- offset from the anchor; y clears the pet frame's bottom edge
        height = 44,   -- total pixel height of this bar (used by petFrame for positioning)
        background = {
            imgTop = {
    ...
    petFrame = {
        ...
        marginBottom = 8,
        background = {
            -- XIV-style background; vertical stack: imgTop (cap), imgMid (body), imgBottom (cap)
            imgTop = {
                path  = 'layouts/assets/xiv/BgTop.png',
                size  = {190, 6},
                pos   = {0, 0},
        ...
        -- HP: anchored to name.bottom so downstream elements stack below name + its margin.
        hp = {
            anchor = 'name.bottom',
            pos    = {0, 0},
        ...
        -- MP: only visible for avatar and automaton.
        mp = {
            anchor = {'hp', 'name.y'},
            pos    = {20, 10},  -- 20px right, 10px below hp (or name) origin
        ...
        -- TP: always visible when pet active.
        tp = {
            anchor = {'mp', 'hp', 'name.y'},
            pos    = {20, 10},  -- 20px right, 10px below anchor origin
            bind    = { visible = 'pet.active' },
            bar = {
                pos       = {0, 0},
                zOrder    = 1,
        ...
        -- Timer: charm/jug countdown (hidden for all other types).
        petTimer = {
            pos    = {15, -5},
            txt    = {
                font        = 'Grammara',
        ...
        -- Recast rows: created dynamically based on number of ability slots.
        recast = {
            bind = {
                visible = 'ui.showRecasts',   -- read by type modules; declared here for reference
            },
            anchor               = {'tp.y', 'mp.y', 'hp.y', 'name.y'},
            rowHeight            = 18,
            firstRowPos          = {0, 18},  -- offset from anchor when pet active
            alwaysShowFirstRowPos = {0, 8},     -- offset from window origin when no pet (alwaysShow mode)
            txt = {
                font        = 'Segoe UI',
                size        = 13,
                bold        = true,
            ...
            timer = {
                ...
                zOrder      = 6,
                thresholds  = {
                    { below = 0.01, color = '#66FF66FF' },  -- green = ready
                    { below = 0.30, color = '#FFBB44FF' },  -- orange = nearly ready
                },
            },
            labelOverrides = {
                ['Stay'] = { text = 'Healing' },
        ...
        -- Status effects icon strip: rendered as a row below all recast rows.
        -- anchor='recast.bottom' resolves to {x=recast.x, y=recast.y+recast.h} (dynamic bottom edge).
        -- Falls back up the chain when no pet type is active (and therefore no recast block).
        statusEffects = {
            anchor       = {'recast.bottom', 'tp', 'mp', 'hp', 'name'},
            maxIcons     = 10,    -- max icons in one row
            iconSize     = 18,    -- icon size in layout pixels (1440p baseline)
            iconGap      = 2,     -- gap between icons
            pos          = { 10, 4 },  -- 10px from left, 4px gap below anchor (recast bottom)
            marginBottom = 4,     -- space below icons (includes timer label height)
            bind = {
                visible    = 'pet.active',       -- hide strip when no active pet
                visibleAnd = 'pet.statusActive',  -- hide strip when no effects (if setting enabled)
            },
            timer = {
                font        = 'Segoe UI',
                size        = 9,    -- font height in layout pixels
                color       = '#FFFFFFFF',
                stroke      = '#000000C8',
                strokeWidth = 1,
                pos         = {0, 20},
        ...
        -- never reach the element hue. Hybrid layout: the panel follows the frame's XIV
        -- background art, while the pip set lives under assets/ffxi/pips/ and is shared by all
        -- layouts -- it is untextured geometry, with no XIV-specific variant.
        maneuvers = {
            bind = {
                visible    = 'pet.active',
            ...
            -- which puts the panel's right edge flush on the frame's left edge; changing an
            -- img width or firstRowPos[1] means re-deriving it.
            --
            ...
            background = {
                pos = {-39, 0},
                imgTop = {
                    path   = 'layouts/assets/xiv/BgTopMirror.png',
                    size   = {59, 6},
                    pos    = {0, 0},
                    color  = '#FFFFFFDD',
                    zOrder = 0,
                ...
                imgMid = {
                    path   = 'layouts/assets/xiv/BgMidMirror.png',
                ...
                imgBottom = {
                    path   = 'layouts/assets/xiv/BgBottomMirror.png',
                    size   = {59, 6},
                    pos    = {0, 24},
                    color  = '#FFFFFFDD',
                    zOrder = 0,
            ...
            -- iconActive also draws pips 2+. The circle sprites in that directory are
        ...
        -- 2hr/familiar ability name label: shown near the timer when a 2h is active.
        special = {
            --anchor = 'window.bottom',
            pos    = {0, 0},
            txt    = {
